...
def analyze_wifi_risk(wifi_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    description = wifi_data.get("description", "")
    ...
    if encryption == "Open" and dns_hijack:
        ...
        intent_analysis = (
            ...
            f"且 DNS 解析存在被劫持的风险。信号强度为 {abs(signal)}dBm，"
    ...
    # ── 规则 2: WEP 加密 或 Open+DNS劫持组合的次高情况 → 中风险 ──
        ...
        # 细分意图分析
        if encryption == "WEP":
            reason = "该网络使用已过时的 WEP 加密协议，此协议可在数分钟内被暴力破解。"
        elif dns_hijack:
            reason = f"该网络使用 {encryption} 加密，但 DNS 解析存在被劫持的风险。"
        else:
            reason = "该网络为开放式网络，未启用加密，数据传输可能被第三方监听。"

        risk_level = "中"
        intent_analysis = (
            f"WiFi \"{ssid}\" 存在中等安全风险。{reason}"
